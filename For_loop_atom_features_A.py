...
from src.smiles_encoder_atom_features import *  #Here change to smiles_encoder_atom_features.py
...
#Output files
# Path
output_checkpoints = "/data/db6/jrivera/Masterthesis/benchmarking/benchmarking_internal/data_output/data_output_model_A/01_checkpoints_global"
output_check_epoch = "/data/db6/jrivera/Masterthesis/benchmarking/benchmarking_internal/data_output/data_output_model_A/01_checkpoints_epochs"
train_png = "/data/db6/jrivera/Masterthesis/benchmarking/benchmarking_internal/data_output/data_output_model_A/01_train_loss_png"
train_table = "/data/db6/jrivera/Masterthesis/benchmarking/benchmarking_internal/data_output/data_output_model_A/01_train_loss_table"
val_png = "/data/db6/jrivera/Masterthesis/benchmarking/benchmarking_internal/data_output/data_output_model_A/01_val_loss_png"
val_table = "/data/db6/jrivera/Masterthesis/benchmarking/benchmarking_internal/data_output/data_output_model_A/01_val_loss_table"
...
def main():
    ...
    #----------------Train data set------------------------#
        ...
        for file_id, file in enumerate(files_train):
            ...
            x, atom_features = build_atom_encoder_tensor(df, property_columns)

            sample_indices = torch.arange(x.shape[0])
            
            dataset = TensorDataset(x, atom_features, sample_indices )
            ...
            for batch_id, batch in enumerate(data_loader):
                ...
                x = batch[0].to(device)
                atom_features = batch[1].to(device)
                indices = batch[1]
                ...
                #1. Apply the selected function to your discrete values and also the properties masking
                masked_x, y_masked, property_labels, property_masks = (
                    apply_masking_function(x, discrete_mask_fn)
                )
                
                #2. Apply the maskinging for the mask atom features
                atom_features = mask_atom_features(
                    atom_features, y_masked
                )
                
                # Run the model
                property_logits, smiles_logits = model(masked_x, atom_features)
        ...
        for file_id, file in enumerate(files_val):
            ...
            with torch.no_grad():
                for batch_id, batch in enumerate(data_loader):
                    ...
                    #3. Apply the maskinging for the mask atom features
                    atom_features = mask_atom_features(
                        atom_features, y_masked
                    )
                    #4. CHeck whether at least one smiles was masked
